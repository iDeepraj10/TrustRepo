...
actual = np.array(s1)                                  #convert pandas dataframe to np array
s1 = np.array(s1)

M_ratings = [[2,10],[2,35],[2,36],[10,5],[10,16],[10,83],[10,20],[18,1],[18,2],[18,3],[18,8],[7,7],[7,9],[7,11]]
...
s1 = pd.DataFrame(s1).T.fillna(round(pd.DataFrame(s1).mean(axis=1),2)).T

s1 = s1.drop(s1.columns[[0]], axis =1)
s1 = s1.drop(s1.columns[[0]], axis =1)
s1.to_csv("C:\\Users\\dexter\\Desktop\\Trust and Reputation\\New folder\\Dataset\\Final Result\\Muvi\\movie_lens_umean.csv")
